...
void ViewManager::ProcessKeyboardEvents()
{
	...
	const float cameraSpeed = 5.0f * gDeltaTime;

	// forward / backward
	if (glfwGetKey(m_pWindow, GLFW_KEY_W) == GLFW_PRESS)
	{
		g_pCamera->Position += g_pCamera->Front * cameraSpeed;
	}
	...
	{
		g_pCamera->Position -= right * cameraSpeed;
	}
...
void ViewManager::PrepareSceneView()
{
    ...
    if (bOrthographicProjection)
    {
        ...
        glm::vec3 target  = glm::vec3(0.0f, 3.0f, 0.0f);
        glm::vec3 up      = glm::vec3(0.0f, 1.0f, 0.0f);

        view = glm::lookAt(camPos, target, up);

        // size of the orthographic "box"
